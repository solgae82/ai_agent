...
# 오케스트레이터 프롬프트 생성
def get_orchestrator_prompt(user_prompt): 
    return f"""
다음 사용자 질문을 분석한 뒤, 이를 3개 이내의 관련 하위 질문으로 분해해.
결과는 JSON 배열로 출력해.
JSON 배열 안의 각 하위 질문은 다음 형식을 따르는 JSON 객체로 만들어.
[
    {{"question": 하위 질문 1", "description":"이 하위 질문의 요지와 의도에 대한 설명"}},
    {{"question": 하위 질문 2", "description":"이 하위 질문의 요지와 의도에 대한 설명"}}
]
...
# streamlit 맞춤형 오케스트레이터-워커 워크플로 실행
async def run_orchestrator_workflow_streamlit(user_query): 
    ...
    parse_response = orchestrator_response.replace("```json","").replace("```","")

    with st.expander("오케스트레이터 응답(JSON 형식)", expanded=True): 
        st.code(parse_response)

    subtask_list = json.loads(parse_response) # [{'question':'..','description':'..'}..]
    ...
    for i, (col, task, prompt, response) in enumerate(zip(
        columns, 
        subtask_list, 
        worker_prompt_details,
        worker_responses
    ...
    aggregator_prompt = (
        ...
        "그리고 반드시 **응답에 활용된 모든 출처는 만지막의 <출처> 섹션에 마크다운 링크 형태로 정리해 제공해줘.**\n"
        f"사용자 질문: {user_query}\n\n"
        "하위 질문 및 응답:\n"
    )
    ...
    with st.expander("aggregator 프롬프트", expanded=False): 
