...
async def run_llm_parallel(prompt_details): 
    tasks = [
        llm_call_async(prompt["user-prompt"], prompt["model"]) for prompt in prompt_details
    ]
...
async def run_parallel_agent(question, selected_models): 
    parallel_prompt_details = [
        {"user-prompt": question, "model": model} for model in selected_models
    ]
...
def main(): 
    ...
    print(question) # value 값, '아래 문장을..'

    model_options = ["gpt-4o", "gpt-4o-mini", 'o3']
    selected_models = st.multiselect(
        "사용할 모델을 선택하세요",
        model_options,
        default=model_options[:3] # 선택박스에 다중 선택된 것 표현됨(현재 3개 모두)
    )

    print(selected_models) # ['gpt-4o', 'gpt-4o-mini', 'o3']
