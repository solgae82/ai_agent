...
if __name__ == "__main__": 
    ...
    for query in queries: 
        ...
        category = llm_router_call(query) #질문 카테고리 분류

        print(f'카테고리 응답=>{category}')
        # 라우팅 맵에 함수 대응 및 호출
        ROUTING_MAP = {
        ...
        final_llm_call = ROUTING_MAP.get(category, run_general_agent) # 응답 키값 받아서 관련 agent 함수로 반환
        response = final_llm_call(query) # 라우팅된 함수로 질문 전달, 받아오기
