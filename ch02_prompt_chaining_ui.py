...
"""추천한 여행지의 하루 일정 계획을 알려줘.
- 오전, 오후, 저녁으로 나눠 일정을 짜줘.
- 각 시간대에 어떤 활동을 하면 좋을지 설명해."""
]

# 1. 프로프트 체이닝 함수 개선
def prompt_chain_workfow(initial_input: str, prompt_chain: List[str]) -> tuple[List[str], List[str]]: 
    response_chain = []
    final_prompts = []
...
처음에 사용자가 입력한 내용은 다음과 같아. 응답할때 항상 이 내용을 고려해.
{initial_input}
...
{previous_response}"""
    ...
    return response_chain, final_prompts # 튜플 패킹 반환
...
def main(): 
    ...
    if st.button("프롬프트 체인 실행"): 
        final_result_tab, details_tab = st.tabs(["최종결과", "세부단계"])

        with st.spinner("실행 중입니다.."): 
            results, final_prompts = prompt_chain_workfow(initial_input,custom_prompts)
